...
class RoadExtremity:
        ...
        if self.spawn_cars:
            ...
            if self.timer >= self.spawn_cars_timer:
                ...
                if self.last_spawned_car:
                    if (self.last_spawned_car.pos -
                            self.pos).length() > self.security_spawn_distance:
    ...
    def get_end_car_pos_dir(self, delta=0):
        if self.road.start_extremity is self:
            return self.road.lanes_start_end_position[0][1].copy(
            ) + self.road.dir * delta, self.road.dir
        else:
            return self.road.lanes_start_end_position[1][1].copy(
            ) - self.road.dir * delta, -self.road.dir
...
class Road:
    ...
    def draw(self, win):
        ...
        p1_transformed = self.simulator.camera.apply(self.start_extremity.pos -
                                                     delta)
        p2_transformed = self.simulator.camera.apply(self.start_extremity.pos +
                                                     delta)
        p3_transformed = self.simulator.camera.apply(self.end_extremity.pos +
                                                     delta)
        p4_transformed = self.simulator.camera.apply(self.end_extremity.pos -
                                                     delta)

        pygame.draw.polygon(
            win, ROAD_COLOR,
            [p1_transformed, p2_transformed, p3_transformed, p4_transformed])

        # left right stripe
        p1_ls_transformed = self.simulator.camera.apply(
            self.start_extremity.pos - delta +
            self.dir_orth * STRIPE_WIDTH / 2)
        p2_ls_transformed = self.simulator.camera.apply(
            self.start_extremity.pos - delta + self.dir_orth *
            (STRIPE_WIDTH / 2 + STRIPE_WIDTH))
        p3_ls_transformed = self.simulator.camera.apply(
            self.end_extremity.pos - delta + self.dir_orth *
            (STRIPE_WIDTH / 2 + STRIPE_WIDTH))
        p4_ls_transformed = self.simulator.camera.apply(
            self.end_extremity.pos - delta + self.dir_orth * STRIPE_WIDTH / 2)

        pygame.draw.polygon(win, (255, 255, 255), [
            p1_ls_transformed, p2_ls_transformed, p3_ls_transformed,
            p4_ls_transformed
        ])

        p1_rs_transformed = self.simulator.camera.apply(
            self.start_extremity.pos + delta -
            self.dir_orth * STRIPE_WIDTH / 2)
        p2_rs_transformed = self.simulator.camera.apply(
            self.start_extremity.pos + delta - self.dir_orth *
            (STRIPE_WIDTH / 2 + STRIPE_WIDTH))
        p3_rs_transformed = self.simulator.camera.apply(
            self.end_extremity.pos + delta - self.dir_orth *
            (STRIPE_WIDTH / 2 + STRIPE_WIDTH))
        p4_rs_transformed = self.simulator.camera.apply(
            self.end_extremity.pos + delta - self.dir_orth * STRIPE_WIDTH / 2)

        pygame.draw.polygon(win, (255, 255, 255), [
            p1_rs_transformed, p2_rs_transformed, p3_rs_transformed,
            p4_rs_transformed
        ])
        ...
        for i in range(1, self.nb_lanes):
            ...
            p1_stripe_transformed = self.simulator.camera.apply(
                self.start_extremity.pos + delta - stripe_delta_world)
            p2_stripe_transformed = self.simulator.camera.apply(
                self.end_extremity.pos + delta - stripe_delta_world)
